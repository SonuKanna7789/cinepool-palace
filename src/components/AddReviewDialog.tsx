...
export function AddReviewDialog({ open, onClose }: ReviewDialogProps) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // 2. Supabase user_reviews — now includes movie_title + poster_path
      const { error: reviewError } = await supabase.from("user_reviews").insert({
        user_id:          user.user_id,
        movie_id:         movieId,
        movie_title:      movieTitle,          // FIX: save title
        poster_path:      posterPath,          // FIX: save poster
      ...
      // 3. Supabase user_watch_history — upsert to avoid duplicates
      const { error: historyError } = await supabase.from("user_watch_history").upsert(
        {
          user_id:     user.user_id,
          movie_id:    movieId,
          movie_title: movieTitle,
          poster_path: posterPath,             // FIX: save poster for watched grid
          genre:       selectedItem.genre_ids?.[0]?.toString() || null,
          rating,
        },
