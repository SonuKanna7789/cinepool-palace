...
async function buildSystemPrompt(
  ...
  const watchHistory = history.map((h: any) => `${h.movie_title}${h.rating ? ` (${h.rating}/5)` : ""}`).join(", ") || "None yet";
  const reviewedMovies = reviews.map((r: any) => `${r.movie_title || "Unknown"} (${r.rating}/5)${r.review_text ? `: "${r.review_text.slice(0, 60)}"` : ""}`).join("; ") || "None yet";
  const likedMovies = feedback.filter((f: any) => f.action === "like").map((f: any) => f.movie_id).join(", ") || "None yet";
  const dislikedMovies = feedback.filter((f: any) => f.action === "dislike").map((f: any) => f.movie_id).join(", ") || "None yet";
...
- Reference their past watches when relevant
